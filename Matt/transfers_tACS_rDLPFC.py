...
tACS_rTPJ_punishment_transfers = []
tACS_rTPJ_non_punishment_transfers = []
tACS_rDLPFC_punishment_transfers = []
tACS_rDLPFC_non_punishment_transfers = []
...
for i in range(0,len(allConditionPermutation)):
    if (allConditionPermutation[i].stimType == "tACS"
        ...
        tACS_rTPJ_punishment_transfers = tACS_rTPJ_punishment_transfers + allConditionPermutation[i].punishmentChoices
        tACS_rTPJ_non_punishment_transfers = tACS_rTPJ_non_punishment_transfers + allConditionPermutation[i].nonPunishmentChoices
    elif (allConditionPermutation[i].stimType == "tACS"
            and allConditionPermutation[i].location == "rDLPFC"):
        tACS_rDLPFC_punishment_transfers = tACS_rDLPFC_punishment_transfers + allConditionPermutation[i].punishmentChoices
        tACS_rDLPFC_non_punishment_transfers = tACS_rDLPFC_non_punishment_transfers + allConditionPermutation[i].nonPunishmentChoices
...
tACS_rTPJ_punishment_transfers1       = np.array(tACS_rTPJ_punishment_transfers)
tACS_rTPJ_non_punishment_transfers1   = np.array(tACS_rTPJ_non_punishment_transfers)
tACS_rDLPFC_punishment_transfers1     = np.array(tACS_rDLPFC_punishment_transfers)
tACS_rDLPFC_non_punishment_transfers1 = np.array(tACS_rDLPFC_non_punishment_transfers)
...
tACS_rTPJ_punishment_mean = np.mean(tACS_rTPJ_punishment_transfers1)
tACS_rTPJ_non_punishment_mean = np.mean(tACS_rTPJ_non_punishment_transfers1)
tACS_rDLPFC_punishment_mean = np.mean(tACS_rDLPFC_punishment_transfers1)
tACS_rDLPFC_non_punishment_mean = np.mean(tACS_rDLPFC_non_punishment_transfers1)
sham_punishment_mean = np.mean(sham_punishment_transfers1)
sham_non_punishment_mean = np.mean(sham_non_punishment_transfers1)

tACS_rTPJ_punishment_std = np.std(tACS_rTPJ_punishment_transfers1)
tACS_rTPJ_non_punishment_std = np.std(tACS_rTPJ_non_punishment_transfers1)
tACS_rDLPFC_punishment_std = np.std(tACS_rDLPFC_punishment_transfers1)
tACS_rDLPFC_non_punishment_std = np.std(tACS_rDLPFC_non_punishment_transfers1)
sham_punishment_std = np.std(sham_punishment_transfers1)
sham_non_punishment_std = np.std(sham_non_punishment_transfers1)

tACS_rTPJ_punishment_std_error = tACS_rTPJ_punishment_std             / 3
tACS_rTPJ_non_punishment_std_error = tACS_rTPJ_non_punishment_std     / 3
tACS_rDLPFC_punishment_std_error = tACS_rDLPFC_punishment_std         / 3
tACS_rDLPFC_non_punishment_std_error = tACS_rDLPFC_non_punishment_std / 3
sham_punishment_std_error = sham_punishment_std         / 3
sham_non_punishment_std_error = sham_non_punishment_std / 3

means = [tACS_rDLPFC_punishment_mean,tACS_rTPJ_punishment_mean,sham_punishment_mean,
         tACS_rDLPFC_non_punishment_mean, tACS_rTPJ_non_punishment_mean, sham_non_punishment_mean]
std_errors = [tACS_rDLPFC_punishment_std_error,tACS_rTPJ_punishment_std_error,sham_punishment_std_error,
             tACS_rDLPFC_non_punishment_std_error,tACS_rTPJ_non_punishment_std_error,sham_non_punishment_std_error]
